...
namespace Assets_Management_System.Services
{
    public class DataSeeder
    {
        ...
        private void SeedTransactions()
        {
            ...
            {
                var asset = assets[i];
                txService.Add(new TransactionClass
                {
                    AssetId = asset.Id,
                    TransactionType = "Assign",
                    EmployeeName = "Sorn Visal",
                    TransactionDate = DateTime.Today.AddDays(-i),
                    Note = "Seeded Transaction"
                });

                // IMPORTANT: Actually change the asset status so Dashboard reacts
                _service.AssignAsset(asset.Id);
            }
